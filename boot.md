...
bind cons.
    ...
    @!18b b8 @!18c 0200 ':: @!190
    @!190 48c7 o300 8c014000 48ab         # push symbol ::

    @!199 e8 6dffffff                     # invoke @:k to generate the closure
    @!19e e8 aeffffff @!1a3               # invoke :: to add binding
...
0x4ffff8. The simplest way to deal with this value in memory is just to build
two special-purpose functions that read or write it.

From here forwards, we use the symbol table to dereference both :: and @:k (plus
whatever other functions we add as we go). The symbol table works; its only
problem is that we need to access it as an anonymous entity for the moment.
...
now. I'm going to stash it onto the return stack for easy access.
    ...
    @!1a3 ff o167 f8 @!1a6                  # symbol table pointer
...
jumping over the definition.
    ...
    @!1a6 eb0b @!1a8
    @!1a8 488b o004 o045 f8ff4f00 48ab c3 # (0x4ffff8) -> %rax; stosq; ret
...
We don't skip over this one for reasons I'll explain in a moment (but the most
obvious reason is that we already have the right value on the stack). The
