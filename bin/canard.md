...
## State tracking and continuations

The reader works incrementally. You can pass it one buffer of input and it
will update its state, but it doesn't need to return a completed parse. This
works because it consumes and produces an explicit continuation that marks its
state. As usual, the initial continuation is represented by nil.

The continuation structure is simple: it's a cons list of intermediate values,
each of which represents one layer of nesting. The consume-one-character
function takes one of these lists and returns another one; the second may
share arbitrarily much structure with the first.

Each intermediate value is a cons that contains the tail of the list being
read. (Remember that the head of a list is written to the right of the tail.)
