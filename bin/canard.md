...
functions those are is up to you.
...
Step ordering is important; they have to occur as listed above. With all of
this stated, here's the behavior of the reader, using string notation to
indicate buffer state. The reader state is always not-inside-symbol anytime $<
is called; it forces the buffer internally until the end of each symbol is
reached. Parentheses specify that something will be evaluated eagerly:
    ...
    $< [l] [s] [f] "[..." [[]]    = $< [l] [s] [f] "..." [[] []]
    $< [l] [s] [f] "]..." [[...]] = l [...]
    $< [l] [s] [f] "]..." [x y]   = $< [l] [s] [f] "..." [x (l y)]

There are a couple more internal rules that deal with things like intermediate
consing, but this covers most of the edge cases. All recursion is done using
tail calls.
...
file has ended. And that's exactly what we do:
    ...
    $< [l] [s] [f] "" [xn ... x2 x1] = l (:: xn ... l (:: x2 (l x1)) ...)

Therefore, we treat an empty buffer as being equivalent to an infinite series
of close brackets. This lets us return useful values from partial parses,
