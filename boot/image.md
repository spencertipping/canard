...
This is the perfect place to introduce a preprocessor macro to save some work.

    - def bind name, value << end
      48c7 o300 $value                         48ab
        c7 o300 $name                          48ab
        e8:4[L:generate_constant_matcher - :>] e8:4[L:cons - :>]
    - end

    - bind :4[Lb:@@:k], :4[Lb:generate_constant_matcher]
    - bind :4[Lb:@@?k], :4[Lb:constant_matcher]
    - bind :4[Lb:@::],  :4[Lb:cons]
    - bind :4[Lb:@@>],  :4[Lb:get_symbol_table]
    - bind :4[Lb:@@<],  :4[Lb:set_symbol_table]

    - bind :4[Lb:@#0],  :4[Lb:syscall0]
    - bind :4[Lb:@#1],  :4[Lb:syscall1]
    - bind :4[Lb:@#2],  :4[Lb:syscall2]
    - bind :4[Lb:@#3],  :4[Lb:syscall3]

Now we have a complete symbol table. Bind it to the global address and start
up the interpreter.

    e8:4[L:set_symbol_table - :>]                 # now it's installed...

    48c7 o300:4[Lb:@main] 48ab                    # push symbol 'main'
    ff o064:4[L:symbol_table - :>] 48ff o156 f8   # resolve/jmp main
...
This is the last piece of the symbol table. We need two more functions; the
first, @<, sets the symbol table pointer to the given function. The second, @>,
retrieves the given symbol table pointer.

Remember from earlier that the symbol table pointer is mutable and is stored at
a fixed address. The simplest way to deal with this value in memory is just to
build two special-purpose functions that read or write it.
...
address onto the data stack.
    ...
    ::get_symbol_table
    488b o004 o045:4[Lb:symbol_table] 48ab c3

## Symbol table setter

This function is only slightly more complex than @> due to the fact that we
don't have a shortcut quite as nice as stosq.

    ::set_symbol_table
    488b o107 f8 4883 o357 08             # data-pop -> %rax;
    4889 o004 o045:4[Lb:symbol_table] c3  # %rax -> (symbol_table); ret
...
%r10, etc, are not saved.
    ...
    ::syscall0
    488b o107 f8 488c o357 08     # data-pop -> %rax
    ...
    488b o167 e8 488c o357 18     # data-pop -> %rsi
...
These use the above and abstract away some of the details of system calling.
    ...
    ::exit
    4831 o300 b03c e8:4[L:syscall0 - :>]
